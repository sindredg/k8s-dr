...
Local checks for Flux and SOPS: `make check` passes: 87 tests, yamllint (now including `deploy/` and `.sops.yaml`), ansible-lint, and syntax checks. `make pins` resolves the Flux 2.9.5 manifest. These checks are not gate evidence.
