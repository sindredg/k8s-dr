...
| Flux and SOPS | `ansible/roles/flux/`, `scripts/run_with_iap.py`, `ansible/playbooks/bootstrap.yml`, `ansible/playbooks/validate_cluster.yml`, `ansible/playbooks/group_vars/all.yml`, `deploy/`, `.sops.yaml`, `Makefile`, `scripts/check_pins.py`, `tests/test_flux.py` | The last bootstrap play applies the pinned Flux 2.9.5 `install.yaml`, creates `flux-system/sops-age` from the operator's key file, and applies an anonymous HTTPS `GitRepository` and a `Kustomization` for `deploy/clusters/primary` with SOPS decryption. The key is read on the operator machine and sent to `kubectl apply` on stdin with `no_log`, so it is never written on the node. `make bootstrap` requires `FLUX_AGE_KEY_FILE` and accepts `FLUX_GIT_BRANCH` for testing a pushed branch. The first Flux-owned objects are the `gitea` and `postgresql` namespaces, annotated so that Flux never prunes them and their volumes. `install.yaml` also runs the unused image automation controllers; filtering them would need the Flux CLI or a patched manifest. Cluster validation now checks the Flux controllers, source, Kustomization, applied revision, and key Secret. The pin check covers the Flux manifest. `.sops.yaml` encrypts `data` and `stringData` in `deploy/**/*.sops.yaml` for one age recipient, and a test fails on any unencrypted Secret under `deploy/`. After applying the key and sync objects, the role requests an immediate reconcile, because Flux does not watch the key Secret. `scripts/run_with_iap.py` sets SSH keepalives for long silent waits. |

Local checks for the public endpoint: `terraform fmt -check` and `terraform validate` pass for the module and the primary root. `make check` passes: 77 tests, yamllint, ansible-lint, and syntax checks. These checks are not gate evidence.

Local checks for Flux and SOPS: `make check` passes: 89 tests, yamllint (now including `deploy/` and `.sops.yaml`), ansible-lint, and syntax checks. `make pins` resolves the Flux 2.9.5 manifest. These checks are not gate evidence.
...
| 2026-09-26 | `make bootstrap FLUX_GIT_BRANCH=feat/flux-bootstrap` at `6060314` | Failed. Control plane `ok=68 changed=13 unreachable=1 failed=0`; `elapsed 264s, exit 4`. The Kustomization reported `malformed secret key: mixed case`. See [troubleshooting](../troubleshooting/03-flux-age-key-rejected.md). |
| 2026-09-26 | Same command at `d86862c` (key fix and SSH keepalives) | Failed. Control plane `ok=68 changed=12 unreachable=0 failed=1`; `elapsed 420s, exit 2`. `kubectl wait` timed out on a failure recorded before the Secret was replaced. |
